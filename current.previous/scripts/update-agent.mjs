...
async function logLine(jobs, job, message, level = 'info') {
  job.logs.push({
    id: `${job.id}-${job.logs.length + 1}`,
    deploymentId: job.id,
    level,
...
async function setJobProgress(jobs, job, currentStep, progressPercent) {
  job.metadata = {
    ...(job.metadata || {}),
    currentStep,
    progressPercent
...
async function applyReleaseMigrations(targetDir, targetManifest, jobs, job) {
  ...
  const appliedNames = new Set(getAppliedMigrationNames())
...
async function startDeployment(version, mode = 'deploy', options = {}) {
  ...
  ;(async () => {
    try {
      ...
      await logLine(jobs, job, 'Downloading release artifact')
      await setJobProgress(jobs, job, 'downloading-artifact', 30)
      ...
      await logLine(jobs, job, `Release extracted to ${targetDir}`)
      await setJobProgress(jobs, job, 'artifact-extracted', 45)
      ...
      await logLine(jobs, job, 'Stopping current runtime')
      await setJobProgress(jobs, job, 'stopping-runtime', 55)
